...
load_version() {
    if [ -z "$VERSION_STR" ] && [ -f "$VERSION_TXT" ]; then
        VERSION_STR=$(sed 's/version://' "$VERSION_TXT")
        NEXT_BUILD=$(echo "$VERSION_STR" | sed -n 's/.*-BUILD-0*\([0-9]*\)/\1/p')
        DATE_TAG="${VERSION_STR%%-BUILD*}"
...
step3_metadata() {
    ...
    VERSION_CODE="${VERSION_CODE:-$NEXT_BUILD}"
    DOWNLOAD_URL="${LARAVEL_APP_URL}/apk/${APP_NAME}-v${VERSION_STR}.apk"
    ...
    cat > "$VERSION_FILE" <<EOF
{
    ...
    "release_date": "${DATE_TAG:-$(date +%Y-%m-%d)}",
...
step6_deploy() {
    ...
    if [ ! -f "$APK_SOURCE" ]; then
        echo "  ⚠ APK not found at $APK_SOURCE, aborting deploy"
        exit 1
    fi
    ...
    sshpass -p "$SSH_PASS" ssh -o StrictHostKeyChecking=no "$SSH_USER@$SSH_HOST" "
      ...
      echo '$SSH_PASS' | sudo -S rm -f $REMOTE_DIR/storage/app/apk-version.json 2>/dev/null || true
    ...
    if [ $# -eq 0 ]; then
        ...
        for s in $(expand_steps "$1"); do
            case $s in
                ...
                5) step5_git ;;
                6) step6_deploy ;;
                *) echo "  ⚠ Unknown step: $s (valid: 1-6)"; exit 1 ;;
            esac
        done
    fi
