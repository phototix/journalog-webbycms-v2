...
class ListsController extends Controller
{
    ...
    public function showList(Request $request)
    {
        ...
        $userIds = $list->members->pluck('id')->toArray();
        if ($userIds) {
            $lastPostDates = Post::whereIn('user_id', $userIds)
                ->where('status', Post::APPROVED_STATUS)
                ->select('user_id', DB::raw('MAX(created_at) as last_posted_at'))
                ->groupBy('user_id')
                ->pluck('last_posted_at', 'user_id');
            $list->members->each(function ($user) use ($lastPostDates) {
                $user->last_posted_at = $lastPostDates[$user->id] ?? null;
            });
        }
