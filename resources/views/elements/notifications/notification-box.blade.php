                    ...
                    @switch($notification->type)
                        @case(\App\Model\Notification::NEW_TIP)
                            @if(isset($notification->transaction))
                                @if($notification->transaction->type === \App\Model\Transaction::GIFT_TYPE)
                                    {{$notification->transaction->sender->name}} {{__("sent you a gift worth")}} {{\App\Providers\SettingsServiceProvider::getWebsiteFormattedAmount(\App\Providers\PaymentsServiceProvider::getTransactionAmountWithTaxesDeducted($notification->transaction))}}.
                                @else
