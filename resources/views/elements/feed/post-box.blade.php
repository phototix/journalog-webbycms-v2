<div class="post-box" data-postID="{{$post->id}}">
    ...
    @if(count($post->attachments))
        <div class="post-media">
            @if(isset($exploreView) && $exploreView)
                ...
                @endif
            @elseif(PostsHelper::isPostSubscriptionUnlocked($post))
    ...
    @else
        @if(isset($exploreView) && $exploreView)
            {{-- In explore mode, text-only posts show text without any lock overlay --}}
        @elseif(PostsHelper::isPostSubscriptionUnlocked($post))
            @if((Auth::check() && Auth::user()->id !== $post->user_id && $post->price > 0 && !PostsHelper::hasUserUnlockedPost($post->postPurchases)) || (!Auth::check() && $post->price > 0 ))
                @include('elements.feed.post-locked',['type'=>'post','post' => $post])
            @endif
    ...
    <div class="post-footer mt-3 pl-3 pr-3">
        <div class="footer-actions d-flex justify-content-between">
            <div class="d-flex">
                @if(!isset($exploreView) || !$exploreView)
                    ...
                    @if(Auth::check() && $post->user->id != Auth::user()->id)
                        @if(PostsHelper::isPostSubscriptionUnlocked($post))
                            <div class="h-pill h-pill-primary send-gift-btn to-tooltip poi {{(!GenericHelper::creatorCanEarnMoney($post->user)) ? 'disabled' : ''}}"
                                 @if(!GenericHelper::creatorCanEarnMoney($post->user))
                                     data-placement="top"
                                 title="{{__('This creator cannot earn money yet')}}">
                            @else
                                data-toggle="modal"
                                data-target="#gift-modal"
                            ...
                            <div class=" d-flex align-items-center">
                                ...
                                <div class="ml-1 d-none d-lg-block"> {{__('Send Gift')}} </div>
                            </div>
                        </div>
                    @else
                        <div class="h-pill h-pill-primary send-gift-btn disabled">
                            <div class=" d-flex align-items-center">
                                @include('elements.icon',['icon'=>'gift-outline', 'variant' => 'medium'])
                                <div class="ml-1 d-none d-md-block"> {{__('Send Gift')}} </div>
                            </div>
                        </div>
                    @endif
                    @endif
            ...
            <div class="mt-0 d-flex align-items-center justify-content-center post-count-details">
                ...
                @endif
                <span class="ml-2-h d-none d-lg-block">
                    <strong class="post-gifts-label-count">{{$post->gifts_count ?? 0}}</strong>
                    <span class="post-gifts-label">{{trans_choice('gifts', $post->gifts_count ?? 0)}}</span>
                </span>
            </div>
        </div>
    </div>
    ...
    {{--    Post gifts received --}}
    @include('elements.gifts.post-gifts', ['post' => $post])
