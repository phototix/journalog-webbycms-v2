<nav class="sidebar {{(Cookie::get('app_theme') == null ? (getSetting('site.default_user_theme') == 'dark' ? '' : 'light') : (Cookie::get('app_theme') == 'dark' ? '' : 'light'))}}">
    ...
    <ul class="list-unstyled menu-elements p-0">
        @if(GenericHelper::isEmailEnforcedAndValidated())
            ...
            <li class="{{Route::currentRouteName() == 'explore' ? 'active' : ''}}">
                <a class="scroll-link d-flex align-items-center" href="{{route('explore')}}">
                    @include('elements.icon',['icon'=>'compass-outline','variant'=>'medium','centered'=>false,'classes'=>'mr-2'])
                    {{__('Explore')}}</a>
            </li>
            ...
            @if(Auth::user()->role_id === 1)
                <li>
                    <a class="scroll-link d-flex align-items-center" href="{{ url('/admin') }}">
                        @include('elements.icon',['icon'=>'shield-checkmark-outline','variant'=>'medium','centered'=>false,'classes'=>'mr-2'])
                        {{__('Admin')}}</a>
                </li>
            @endif
